  ...
  describe('Constructor Functionality', () => {
    it('should add bun and ingredient to constructor', () => {
      ...
      cy.get(selectors.addBlueBunButton)
        .click()
        .then(() => {
          cy.get(selectors.topBunSelector).contains('Краторная булка N-200i').should('exist');
          cy.get(selectors.bottomBunSelector).contains('Краторная булка N-200i').should('exist');
        });

      cy.get(selectors.addMainIngredientButton).click();

      cy.get(selectors.ingredientBeetwen).contains('Соус Spicy-X').should('exist');
    });
